...
module.exports = {
	...
	baseApi(sub_url, method, json_data, cb) {
		let request = {
			...
			headers: {
				...
				Authorization: Cache.ACCESS_TOKEN ? 'Bearer ' + Cache.ACCESS_TOKEN : null,
				Accept: '*/*'
			}
		};
	...
	async uploadImage(uri, type, cb) {
		try {
			...
			let response = await fetch(config.SERVICE_API_URL + 'media', {
				method: 'POST',
				headers: {
					...
					Authorization:
						'Bearer ' +
						'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xLOg-55dltQAUxdDywmrU9d2l4L3z_QqS4PyIklWbvM'
				},
				body: formData
			});
	...
	addJob(job_name, job_details, job_task, job_type, job_start, job_end, estimated_cost, rate, cb) {
		this.baseApi(
			...
			'POST',
			{
				...
				job_details,
				job_task,
				job_ads_type: job_type,
				start_time: job_start,
				end_time: job_end,
				request_vacancy: estimated_cost,
				rate
			},
			cb
		);
	...
	addNote(note_field_id, title, note, date, media_id, cb) {
		media_id !== [] &&
			this.baseApi(
				`campos/gis/${note_field_id}/notes`,
				'POST',
				{ title, date, media_id, note, timeoffset: 3 },
				cb
			);
		media_id == [] &&
			this.baseApi(`campos/gis/${note_field_id}/notes`, 'POST', { title, date, note, timeoffset: 3 }, cb);
	},
	...
	addTask(task_field_id, title, date_from, date_to, description, media_id, geo_tag, assigned_to, supervised_by, cb) {
		media_id &&
			this.baseApi(
				`campos/gis/${task_field_id}/tasks`,
				'POST',
				{ title, date_from, date_to, description, media_id, geo_tag, assigned_to, supervised_by },
				cb
			);
		!media_id &&
			this.baseApi(
				`campos/gis/${task_field_id}/tasks`,
				'POST',
				{ title, date_from, date_to, description, geo_tag, assigned_to, supervised_by },
				cb
			);
	},
