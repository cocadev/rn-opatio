...
export default class RefreshFavListItem extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <View style={{flex:2, paddingVertical: normalize(6)}}>
        <Image source={{ uri: item.picture.medium }} style={styles.circle} />
      ...
      <View style={{position:'relative', flex:4, paddingLeft:width/12, justifyContent:'space-between'}}>
          <Text style={i.smallText}>{item.location.timezone.description}</Text>
          ...
          <View>
            ...
            <Text style={i.smallText}>S({item.location.postcode})</Text>
          </View>

          <View style={{flexDirection:'row'}}>
            <View style={{flex:1}}>
              <Text style={[text.m_8_dark2, ]}>SELLER</Text>
              <Text style={[i.normalText, {color:colors.GREEN, fontSize:10,}]}>{item.name.first + ' ' + item.name.last}</Text>
            </View>
            <View style={{flex:1, marginLeft:12}}>
              <Text style={[text.m_8_dark2]}>PRICE</Text>
              <Text style={[i.normalText, {color:colors.GREEN, fontSize:10}]}>${item.dob.age} / Hour</Text>
            </View>
          </View>
